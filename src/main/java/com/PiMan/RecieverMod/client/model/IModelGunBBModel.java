...
public class IModelGunBBModel implements IModel {
	...
	public IModelGunBBModel(@Nullable ResourceLocation baseLocation) {
    ...
    public static List<ResourceLocation> getParts(ResourceLocation loc) {
		...
		Gson gson = new Gson();
		try {
			ResourceLocation location = new ResourceLocation(Reference.MOD_ID, loc.getResourcePath());
    ...
    public IBakedModel bakeModelPart(IModelState state, final VertexFormat format, Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter, ResourceLocation location)
    {
		...
		if (location != null) {
			...
			try {
				InputStream in = Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation(Reference.MOD_ID, "models/item/" + location.getResourcePath().replace("-", ""))).getInputStream();
				BufferedReader reader = new BufferedReader(new InputStreamReader(in));
				model = ModelBlock.deserialize(reader);
				reader.close();
	...
	public IBakedModel bakeNormal(ModelBlock model, IModelState perState, final IModelState modelState, List<TRSRTransformation> newTransforms, final VertexFormat format, final Function<ResourceLocation, TextureAtlasSprite> bakedTextureGetter, boolean uvLocked) {
        ...
        //TextureAtlasSprite particle = bakedTextureGetter.apply(new ResourceLocation(model.resolveTextureName("0")));
        com.PiMan.RecieverMod.client.model.SimpleBakedBBModel.Builder builder = new SimpleBakedBBModel.Builder(model, model.createOverrides(), bakedTextureGetter);
        
        return new PerspectiveMapWrapperGun(builder.build(), perState);

	}

	@Override
    public IModelGunBBModel process(ImmutableMap<String, String> customData)
    {
		
		//System.out.println("Model Process");
		
		
        return new IModelGunBBModel(baseLocation);
    }
	
	@Override
	public IModelGunBBModel retexture(ImmutableMap<String, String> textures)
    {
        ...
        return new IModelGunBBModel(base);
    }
	...
	protected static BufferedReader getModelReader(ResourceLocation modelLocation) {
		...
		try {
			InputStream in = Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation(Reference.MOD_ID, "models/item/" + modelLocation.getResourcePath())).getInputStream();
			BufferedReader reader = new BufferedReader(new InputStreamReader(in));
			return reader;
	...
	protected BakedQuad makeBakedQuad(BlockPart blockPart, BlockPartFace blockPartFace, TextureAtlasSprite textureAtlasSprite, EnumFacing enumFacing, net.minecraftforge.common.model.ITransformation transformation, boolean uvLocked)
    {
        return new FaceBakery().makeBakedQuad(blockPart.positionFrom, blockPart.positionTo, blockPartFace, textureAtlasSprite, enumFacing, transformation, blockPart.partRotation, uvLocked, blockPart.shade);
    }
	...
	public enum GunModelLoader implements ICustomModelLoader {
		...
		public boolean accepts(ResourceLocation modelLocation) {
			boolean flag = modelLocation instanceof ModelResourceLocation && modelLocation.getResourceDomain().equals(Reference.MOD_ID) && modelLocation.getResourcePath().endsWith(".bbmodel");
			return flag;
		}

		@Override
		public IModel loadModel(ResourceLocation modelLocation) throws Exception {
			Main.LOGGER.info("Loading BBModel: " + modelLocation);
			return new IModelGunBBModel(modelLocation);
		}
	}
	...
	private static final class BakedModelGun implements IBakedModel
    {
        ...
        public BakedModelGun(IModelGunBBModel parent,
