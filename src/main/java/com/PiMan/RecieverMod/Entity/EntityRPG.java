...
public class EntityRPG extends Entity implements IProjectile, IThrowableEntity {
    ...
    public EntityRPG(World worldIn)
    {
        super(worldIn);
        this.xTile = -1;
        this.yTile = -1;
    ...
    public EntityRPG(World worldIn, double x, double y, double z)
    {
        this(worldIn);
        this.setPosition(x, y, z);
    }

    public EntityRPG(World worldIn, EntityLivingBase shooter)
    {
        this(worldIn, shooter.posX, shooter.posY + (double)shooter.getEyeHeight(), shooter.posZ);
    ...
    protected void onHit(RayTraceResult raytraceResultIn)
    {
        ...
        if (entity != null)
        {
            ...
            {
                damagesource = new EntityDamageSourceIndirect("rpg", this, this);
            }
            else
            {
                damagesource = new EntityDamageSourceIndirect("rpg", this, this.shootingEntity);
            }
        ...
        else
        {
            ...
            if (iblockstate.getMaterial() != Material.AIR)
            {
                ...
                this.arrowHitBlock(this.world, blockpos.offset(raytraceResultIn.sideHit));
            }
        }
    }
    ...
    protected void arrowHit(EntityLivingBase living) {
    	this.explode(living.world, new BlockPos(living), this.getLookVec());
    }
    
    protected void arrowHitBlock(World world, BlockPos pos) {
    	this.explode(world, pos, this.getLookVec());
    }
    
    protected void explode(World world, BlockPos pos, Vec3d direction) {
    	if (!world.isRemote) {
    		int length = 4;
    		float size = 2;
    		if (this.getEntityData().getBoolean("admin")) {
    			length = 20;
    			size = 10;
    		}
	    	for (int i = 0; i < length; i++) {
	    		world.createExplosion(null, pos.getX() + 0.5 - direction.x * i, pos.getY() + 0.5 - direction.y * i, pos.getZ() + 0.5 + direction.z * i, size, true);
	    	}
	    	this.setDead();
    	}
    }
